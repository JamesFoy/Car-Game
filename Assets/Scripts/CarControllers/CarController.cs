    ...
    void FixedUpdate()
    {
        ...
        if (thisNumber == PlayerNumber.p1) //This is setup in the editor to be p1
        {
            ...
            if (controllerSetup.state1.IsConnected)
            {
                ...
                {
                    GetComponent<CarAbilityDefiner>().AbilityTrigger(AbilityDeployModes.DeployStyle.Attack);
                }
                if (controllerSetup.state1.Buttons.LeftShoulder == ButtonState.Pressed)
                {
                    GetComponent<CarAbilityDefiner>().AbilityTrigger(AbilityDeployModes.DeployStyle.Defense);
                }
            }
            #endregion
            #region Keyboard
